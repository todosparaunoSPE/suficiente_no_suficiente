...
if uploaded_file is not None:
    ...
    if all(col in df.columns for col in required_columns):
        ...
        df_resultados['Combinación'] = df_resultados['Evaluación de la pensión'] + ', ' + df_resultados['Estado de jubilación']

        # Filtrar las combinaciones que realmente existen en los datos
        combinaciones_validas = df_resultados['Combinación'].value_counts().reset_index()
        combinaciones_validas.columns = ['Combinación', 'Número de Trabajadores']
        
        # Filtro múltiple para la columna "Combinación"
        combinacion_filter = st.multiselect(
            "Filtrar por Combinación de Evaluación y Estado de Jubilación", 
            options=combinaciones_validas['Combinación'].unique(),
            default=combinaciones_validas['Combinación'].unique()
        )
        
        df_filtrado_combinacion = df_resultados[df_resultados['Combinación'].isin(combinacion_filter)]

        st.write("Resultados filtrados por combinación:")
        st.write(df_filtrado_combinacion)

        # Gráfico de barras para "Combinación de Evaluación y Estado de Jubilación"
        combinacion_counts = df_filtrado_combinacion['Combinación'].value_counts().reset_index()
        ...
        fig2 = px.bar(
            ...
            y='Número de Trabajadores',
            color='Combinación',
            labels={'Combinación': 'Combinación de Evaluación y Estado de Jubilación', 'Número de Trabajadores': 'Cantidad'},
            title="Distribución por Combinación de Evaluación y Estado de Jubilación"
        )
        ...
        # Resumen debajo del segundo gráfico
        resumen_combinacion = df_filtrado_combinacion['Combinación'].value_counts().reset_index()
        resumen_combinacion.columns = ['Combinación', 'Número de Trabajadores']

        st.write("### Resumen de Combinación de Evaluación y Estado de Jubilación:")
        for index, row in resumen_combinacion.iterrows():
            st.write(f"{row['Combinación']}: {row['Número de Trabajadores']}")
